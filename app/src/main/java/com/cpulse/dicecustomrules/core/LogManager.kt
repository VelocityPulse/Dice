...
object LogManager {
    ...
    fun debug(iMessage: String) {
        if (ENABLED_LOG && !sSilentDebugLog) debug(
            UNKNOWN_TAG,
            iMessage
        )
    }
    ...
    fun debug(iTag: String, iMessage: String) {
        if (ENABLED_LOG && !sSilentDebugLog) Log.d(
            TAG_PREFIX,
            "$iTag : $iMessage"
        )
    }
